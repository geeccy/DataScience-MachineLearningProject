...
The model of 53 variables as 15699 observations took almost 1 hour to build.
```{r}
...
```{r}
pred_train_rf <- predict(modelrf, trainsub[,-54])
confusionMatrix(pred_train_rf, trainsub[,54])
#Accuracy 100%
```
...
```{r}
pred_test_rf <- predict(modelrf, testing)
```
...
t(data.frame(pred_test_rf, pred_test_gbm, pred_test_treebag))
```
